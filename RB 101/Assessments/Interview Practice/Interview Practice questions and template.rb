...
def longest(string)
  current_substring = ""
  characters = string.chars
  longest_substring = characters[0] # the longest substring has to be at least one character
  # so long as the input is a string
  characters.each_with_index do |start_letter, n| # start at each letter
    position = n
    current_substring = start_letter
    until position == (string.size - 1)
      break if characters[position] > characters[position + 1] # if two consecutive letters
      # are out of alpha order, break
      position += 1
      current_substring << characters[position]
    end
    longest_substring = current_substring if current_substring.size > longest_substring.size
    # only reasign longest_substring when, through the iterations we encounter a longer current_substring.
  end
  longest_substring
end

def substrings(str) # this is from friend Cruz
  subs = []
  (0...str.size).each do |st| # 0
    (st...str.size).each do |last| # 0, 1, 2, 3  [0..0] [0..1] [0..2] [0..3]
      subs << str[st..last]
    end
  end
  subs
...
just_letters = #select out only the alpha characters, (if the character is an element
of a regex expression, or just an array of all the letters)
map through the original, check if letter, then replace with corresponding swapped letter, else skip that character
...
def reverse_only_letters(string)
  all_characters = string.chars
  letters = /[a-zA-Z]/
  alpha_only = all_characters.select { |x| x =~ letters}
