...
SELECT quantity, p.name from orders join products p on p.id = orders.product_id;
...
/*
 note that a foreign key constraint *does* allow for null values
 (this is dumb, right?) so 'not null' needs to be used in conjunction
 */
