...
# arr = [Length.new(1, :mi), Length.new(1, :nmi), Length.new(1, :km)]
# # puts arr[1].class.ancestors
# # puts arr[1] >= arr[2]
# # puts arr[0] < arr[2]
# # puts arr[0] >= arr[1]
# puts arr.sort
...
class BankAccount
  ...
  def withdraw(amount)
    if amount > 0 && valid_transaction?(balance - amount)
      self.balance -= amount
      # weird does it just revert to 50 - 80 even though it doesn't change the instance variable balance?
      # yes, because setter methods always return their caller
